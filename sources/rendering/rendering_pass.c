...
/*   Updated: 2025/11/11 11:02:11 by nkhamich         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_ray(t_data *data, t_ray *ray, int32_t y, int32_t x)
{
	...
	ray->hit_data->color = new_color(1.0f, 1.0f, 1.0f);
}
...
void	get_first_hit(t_data *data, t_ray *ray, int32_t y, int32_t x)
{
	...
	{
		data->pixels[y][x].final_color = ray->hit_data->color;
		gamma_adjust(&data->pixels[y][x].final_color);
		return ;
	}
	data->pixels[y][x].obj_color = ray->hit_data->obj_color;
	...
	while ((int)y_start < data->scene.height)
	{
		...
		while (++x < data->scene.width)
		{
			...
			mlx_put_pixel(data->image_buffer, x, y_start,
				percentage_to_rgba(data->pixels[y_start][x].final_color));
		}
		y_start += y_stride;
	}
}
